    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          ...
          // Line Chart
          if (!state.isLoading && state.reports.isNotEmpty)
            AdminTransactionChart(reports: state.reports),
...
class AdminTransactionChart extends StatelessWidget {
  final List<TransactionReportEntity> reports;

  const AdminTransactionChart({super.key, required this.reports});

  @override
  Widget build(BuildContext context) {
    // 1. Group data by the last 7 days
    ...
    // Round maxVal up to nearest multiple of 5 for nice grid lines
    maxVal = ((maxVal + 4) ~/ 5) * 5;
